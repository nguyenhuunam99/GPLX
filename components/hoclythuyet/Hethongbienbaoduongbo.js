...
export default class Hethongbienbaoduongbo extends Component {
    constructor(props) {
        super(props);
        this.state = {
            question: [],
    ...
    componentDidMount() {
        ...
        // })
        db.transaction(tx => {
            tx.executeSql('SELECT * FROM question WHERE ZQUESTIONTYPE=48 ORDER BY Z_PK LIMIT 36', [], (tx, results) => {
                var temp = [];
        ...
        if (answer.length == 1) {
            ...
            if(click.search(opt)==-1){
               ...
               //     var learn;
                    ...
                    if(temp_click.length==1) {temp_click="0";learn=0}
